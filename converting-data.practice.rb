...
hash = {}
array.map{|sub| hash[sub[0]] = sub[1]}
p hash
...
hash = {}
array.map{|item| hash[item[:id]] = item}
p hash
...
string = "bookkeeper"
hash = {}
string.split('').map{|a| hash[a] = string.count(a)}
p hash
...
hash = hash.map{|k, v| [id: k, name: v[:name], age: v[:age]]}
p hash
...
times_appears = {}
words.map do |word| 
  unless times_appears[word]
    times_appears[word] = 1
  else
    times_appears[word] += 1
  end
end
...
products_hash = {}
products.map{|product| products_hash[product[:name]] = {color: product[:color], weight: product[:weight], price: prices[product[:name]]}}
p products_hash
